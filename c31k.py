...
CATEGORIES = {"vpns": "VPN", "tools": "TOOLS", "games": "APPS"}
NAME_MAX = 40
...
def current_badge():
    ...
    conn.close()
    return row["badge"] if row and row["badge"] else ""


def is_moderator():
    return is_admin() or current_badge() == "moderator"
...
def admin_set_badge():
    ...
    if not user:
        ...
        conn.execute("UPDATE users SET badge=? WHERE id=?", (badge, user["id"]))
        conn.commit()
        flash("badge_saved")
    conn.close()
    return redirect(url_for("admin"))
...
<script>
(function(){
  let state=null, img=new Image(), activeShape='circle', activeCb=null, wired=false;
  const SIZE=280;
  function els(){
  ...
  window.C31Crop=function(file,opts,cb){
    activeShape=(opts&&opts.shape)||'circle';activeCb=cb;
  ...
  <div class="right">
    ...
    <div class="menu-wrap">
      <button class="profile-btn" id="pbtn" type="button" aria-expanded="false" aria-haspopup="true">
        ...
        <span>{{ t('profile') }}</span>{% if me.badge %}<img class="badge-ic" src="{{ badge_icon(me.badge) }}" alt="" title="{{ t(me.badge + '_badge') }}">{% endif %}<span class="caret">▾</span>
      </button>
      <div class="menu" id="pmenu" hidden>
        <button class="item" type="button" data-dlg="dProfile">{{ t('profile') }}{% if me.badge %}<img class="badge-ic" src="{{ badge_icon(me.badge) }}" alt="" title="{{ t(me.badge + '_badge') }}">{% endif %}</button>
...
<dialog id="dProfile" class="prof">
  ...
  <h2 style="margin:0">{{ me.display_name }}{% if me.badge %} <img class="badge-ic" src="{{ badge_icon(me.badge) }}" alt="" title="{{ t(me.badge + '_badge') }}">{% endif %}</h2>
  <p class="muted" style="margin:4px 0 0">@{{ me.username }}{% if is_admin %} · {{ t('admin_badge') }}{% endif %}{% if me.badge %} · {{ t(me.badge + '_badge') }}{% endif %}</p>
...
<div class="panel">
  ...
  <div class="arow">
    <div><strong>{{ u.display_name }}</strong><small>@{{ u.username }} · {{ t(u.badge + '_badge') }}</small></div>
    <form method="post" action="{{ url_for('admin_set_badge') }}">
      <input type="hidden" name="username" value="{{ u.username }}"><input type="hidden" name="badge" value="">
      <button class="btn danger sm">{{ t('remove_badge') }}</button>
    </form>
  </div>
  {% endfor %}
</div>
...
{% block content %}<main class="wrap">
<div class="panel" style="margin-top:20px">
  <span class="eyebrow">{{ t('mod_panel')|upper }}</span><h1>{{ t('ban_user') }}</h1>
  <form method="post">
...
<div class="panel" style="margin-top:20px">
  ...
  <div class="arow">
    <div style="display:flex;align-items:center;gap:12px">
      <span class="mini" style="width:36px;height:36px;border-radius:11px">{% if u.avatar %}<img src="{{ media(u.avatar) }}" alt="">{% else %}{{ u.display_name[:1]|upper }}{% endif %}</span>
      <div><strong>{{ u.display_name }}</strong>{% if u.badge %} <img class="badge-ic" src="{{ badge_icon(u.badge) }}" alt="" title="{{ t(u.badge + '_badge') }}">{% endif %}<br><small class="muted">@{{ u.username }}</small></div>
    </div>
  </div>
  {% endfor %}
</div>
