...
# ═══════════════════════════════════════════════════════════════
#  ADMIN HESABI  →  Sitede kayıt olurken kullandığın isim buraya yazılacak.
#  Bu isimle giriş yapan hesap admin olur (sağ üstte "Admin Panel" görünür).
#  (Render'da ADMIN_USER ortam değişkeni verirsen o, kod içindeki değerin yerine geçer.)
# ═══════════════════════════════════════════════════════════════
AdminUser = "Username"
ADMIN_USER = os.environ.get("ADMIN_USER") or AdminUser
...
STR = {
    "en": {
        ...
        "eyebrow": "// ARCHIVE", "tagline": "Everything on the site, in one search.",
        "search_ph": "Search: games, tools, vpn...", "search": "Search",
        "items": "items", "no_results": "Nothing matches your search.", "no_items": "No items yet.",
        "open": "Open", "download": "Download", "new": "NEW", "online": "online",
        "online_now": "online right now",
        ...
        "change_name": "Change name", "delete_account": "Delete account", "sign_out": "Sign out",
        "theme": "Theme", "dark": "Black", "light": "White", "language": "Language",
        "save": "Save", "cancel": "Cancel", "display_name": "Display name", "picture": "Picture",
        ...
        "lock_btn": "Disable site for everyone", "lock_now": "Site is disabled until",
        "lock_open": "Re-open now", "lock_note": "Everyone except you (admin) will see the closed page.",
        ...
        "added": "Published.", "deleted": "Deleted.", "lock_set": "Site disabled.",
        "lock_cleared": "Site re-opened.", "lock_bad": "Pick a time in the future.",
    },
    "tr": {
        "made_by": "Made by: Antalya Obbyists & Dashers",
        "eyebrow": "// ARŞİV", "tagline": "Ne aratırsan, sitedeki her şey tek aramada.",
        "search_ph": "Ara: oyun, tool, vpn...", "search": "Ara",
        "items": "içerik", "no_results": "Aramana uygun içerik bulunamadı.", "no_items": "Henüz içerik yok.",
        "open": "Aç", "download": "İndir", "new": "YENİ", "online": "online",
        "online_now": "kişi şu an online",
        ...
        "change_name": "İsim değiştir", "delete_account": "Hesabı sil", "sign_out": "Çıkış yap",
        "theme": "Tema", "dark": "Siyah", "light": "Beyaz", "language": "Dil",
        "save": "Kaydet", "cancel": "Vazgeç", "display_name": "Görünen isim", "picture": "Resim",
        ...
        "lock_btn": "Siteyi herkes için kapat", "lock_now": "Site şu zamana kadar kapalı:",
        "lock_open": "Şimdi tekrar aç", "lock_note": "Sen (admin) hariç herkes kapalı sayfayı görür.",
        ...
        "added": "Yayınlandı.", "deleted": "Silindi.", "lock_set": "Site kapatıldı.",
...
def is_admin():
    return bool(session.get("username")) and session["username"] == ADMIN_USER
...
# ───────────────────────── site kilidi + anlık kullanıcı sayacı ─────────────────────────
@app.before_request
def gate():
...
@app.before_request
def track():
    if request.endpoint not in {None, "static", "uploads", "favicon", "set_lang"}:
        touch_active()
...
def inject():
    ...
    conn.close()
    return {"active_count": online, "me": me, "categories": CATEGORIES, "counts": counts,
...
def register():
    if request.method == "POST":
        ...
        # İsim ve şifre için artık uzunluk / karakter zorunluluğu yok (sadece boş olamaz)
        if not username:
            flash("name_empty")
...
def delete_account():
    ...
    conn.execute("DELETE FROM active_users WHERE username=?", (username,))
    conn.commit()
    conn.close()
    if user:
        remove_upload(user["avatar"])
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if kind == "link":
                ...
                source = save_upload(request.files.get("source_file"), "content", FILES)
                if not source:
...
:root{color-scheme:dark;--bg:#0a0709;--veil:rgba(8,5,7,.80);--panel:rgba(20,13,16,.86);--solid:#150e11;--line:rgba(255,70,80,.22);--text:#f6eff1;--muted:#a99ca1;--a:#ff3b47;--b:#ff7a4d;--danger:#ff5d7a;--field:#0d080a;--nav:rgba(10,6,8,.82);--thumb:#1a1013;--on-a:#fff;--shadow:rgba(0,0,0,.5);--glow:rgba(255,59,71,.18)}
html[data-theme=light]{color-scheme:light;--bg:#f7f2f2;--veil:rgba(255,255,255,.80);--panel:rgba(255,255,255,.90);--solid:#fff;--line:rgba(160,20,30,.22);--text:#1c1114;--muted:#6f5f64;--a:#d1141f;--b:#f0562f;--danger:#e11d48;--field:#faf6f6;--nav:rgba(255,255,255,.86);--thumb:#eadcdc;--on-a:#fff;--shadow:rgba(60,20,25,.22);--glow:rgba(209,20,31,.12)}
...
.brand img{width:38px;height:38px;border-radius:11px;object-fit:cover;box-shadow:0 0 0 1px var(--line),0 0 22px var(--glow);transition:transform .3s}
.brand:hover img{transform:rotate(-8deg) scale(1.08)}
.nav{display:flex;gap:4px;flex:1}
.nav a{position:relative;padding:8px 14px;border-radius:10px;color:var(--muted);transition:color .2s,background .2s,transform .2s}
.nav a sup{margin-left:5px;font-size:10px;padding:1px 6px;border-radius:99px;background:var(--glow);color:var(--a)}
...
.nav a:hover{color:var(--text);background:var(--glow);transform:translateY(-2px)}
.nav a:hover::after,.nav a.on::after{width:60%}.nav a.on{color:var(--text)}
...
.profile-btn .nm{max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.menu{position:absolute;right:0;top:calc(100% + 10px);min-width:250px;padding:8px;border:1px solid var(--line);border-radius:16px;background:var(--solid);box-shadow:0 24px 70px var(--shadow);animation:rise .2s both}
...
.opts button,.opts a{flex:1;text-align:center;padding:8px 10px;border:1px solid var(--line);border-radius:10px;background:transparent;color:var(--muted);cursor:pointer;font-size:13px;transition:.2s}
...
.btn:hover{transform:translateY(-2px);background:linear-gradient(135deg,var(--a),var(--b));border-color:transparent;color:var(--on-a);box-shadow:0 10px 30px var(--glow)}
.btn:active{transform:scale(.97)}
.btn.full{width:100%}
.btn.solid{background:linear-gradient(135deg,var(--a),var(--b));border-color:transparent;color:var(--on-a)}
...
.hero h1{font-size:clamp(44px,9vw,84px);line-height:1;margin:14px 0 12px;letter-spacing:-2px}
.hero p{margin:0}
.search{position:relative;display:flex;align-items:center;gap:8px;max-width:640px;margin:30px auto 0;padding:7px;border:1px solid var(--line);border-radius:18px;background:var(--panel);backdrop-filter:blur(12px);box-shadow:0 20px 60px var(--shadow);transition:border-color .25s,box-shadow .25s,transform .25s}
...
/* liste satırları (demon list tarzı) */
.list{display:grid;gap:14px;margin-top:26px}
.rc{position:relative;display:flex;min-height:150px;border:1px solid var(--line);border-radius:16px;overflow:hidden;background:#150c0f;color:#fff;animation:rise .5s cubic-bezier(.2,.7,.2,1) both;animation-delay:calc(min(var(--i,0),12)*45ms);transition:transform .25s,border-color .25s,box-shadow .25s}
@keyframes rise{from{opacity:0;transform:translateY(14px)}}
.rc:hover{transform:translateY(-3px);border-color:var(--a);box-shadow:0 18px 50px var(--shadow)}
.rc-thumb{position:relative;flex:0 0 min(320px,38%);background:var(--thumb);display:grid;place-items:center;color:rgba(255,255,255,.25);font-weight:700;font-size:26px;letter-spacing:8px;overflow:hidden}
.rc-thumb img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;transition:transform .5s}
.rc:hover .rc-thumb img{transform:scale(1.06)}
.rank{position:absolute;left:14px;top:14px;z-index:2;padding:4px 10px;border-radius:8px;background:rgba(130,20,32,.88);color:#fff;font-size:13px;font-weight:700;letter-spacing:.5px;letter-spacing:0}
.newtag{position:absolute;left:14px;bottom:14px;z-index:2;padding:2px 9px;border-radius:7px;background:linear-gradient(135deg,var(--a),var(--b));font-size:10px;font-weight:700;letter-spacing:1.2px;color:#fff}
...
.rc-main::before{content:"";position:absolute;inset:0;z-index:1;background:linear-gradient(90deg,rgba(10,4,6,.92) 0%,rgba(60,8,16,.62) 55%,rgba(110,14,26,.42) 100%)}
.rc-txt,.rc-side{position:relative;z-index:2}
.rc-txt{min-width:0}
.rc-txt h3{margin:0;font-size:clamp(22px,3.2vw,36px);line-height:1.1;letter-spacing:-.5px;overflow:hidden;text-overflow:ellipsis;text-shadow:0 2px 14px rgba(0,0,0,.6)}
.rc-txt .sub{margin-top:8px;font-size:14px;color:rgba(255,255,255,.75)}
.rc-txt .sub b{color:#ff8a92;font-weight:500}
.rc-side{display:flex;align-items:center;gap:14px;text-align:right;flex:none}
.rc-side .act{display:grid;line-height:1.15}
.rc-side .act strong{font-size:20px;color:#ff9aa1}.rc-side .act small{font-size:11px;color:rgba(255,255,255,.6)}
.rc-side .arrow{font-size:16px;color:rgba(255,255,255,.75);transition:transform .25s}
.rc:hover .arrow{transform:translateX(4px);color:#fff}
.empty{margin-top:30px;padding:60px 20px;text-align:center;border:1px dashed var(--line);border-radius:20px;color:var(--muted);background:var(--panel)}
...
.panel{width:100%;max-width:680px;margin:auto;padding:30px;border:1px solid var(--line);border-radius:22px;background:var(--panel);backdrop-filter:blur(14px);box-shadow:0 30px 90px var(--shadow);animation:rise .5s both}
.panel.sm{max-width:390px;text-align:center}.panel.sm form,.panel.sm p{text-align:left}
.panel+.panel{margin-top:18px}
.logo{width:68px;height:68px;border-radius:20px;object-fit:cover;box-shadow:0 0 0 1px var(--line),0 0 40px var(--glow)}
...
.lang-float{position:fixed;right:14px;bottom:14px;z-index:20}
...
@media(max-width:760px){.top{flex-wrap:wrap;gap:8px}.nav{order:3;width:100%;overflow-x:auto}.pill,.profile-btn .nm,kbd{display:none}.two{grid-template-columns:1fr}.hero{padding-top:30px}.brand span{font-size:12px;max-width:140px}.rc{flex-direction:column}.rc-thumb{flex:0 0 130px}.rc-main{padding:16px 18px}}
...
THEME_INIT = "<script>try{document.documentElement.dataset.theme=localStorage.getItem('theme')||'dark'}catch(e){}</script>"
...
<header class="top">
  ...
  <div class="right">
    {% if me %}
    <span class="pill"><i></i><b data-online>{{ active_count }}</b>&nbsp;{{ t('online') }}</span>
    ...
    <div class="menu-wrap">
      <button class="profile-btn" id="pbtn" type="button" aria-expanded="false" aria-haspopup="true">
        ...
        <span class="nm">{{ t('profile') }}</span><span class="caret">▾</span>
      </button>
      <div class="menu" id="pmenu" hidden>
        ...
        <div class="opts"><button type="button" data-theme="dark">{{ t('dark') }}</button><button type="button" data-theme="light">{{ t('light') }}</button></div>
...
{% if me %}<footer class="foot">{{ t('made_by') }} · <b data-online>{{ active_count }}</b> {{ t('online_now') }}</footer>
...
{% endif %}
<script>
function setTheme(t){document.documentElement.dataset.theme=t;try{localStorage.setItem('theme',t)}catch(e){}document.querySelectorAll('[data-theme]').forEach(b=>b.classList.toggle('on',b.dataset.theme===t))}
...
{% block content %}<main class="wrap">
  <section class="hero">
    ...
    <form class="search" method="get" action="{{ url_for('index') }}">
      ...
      <input name="q" value="{{ q }}" placeholder="{{ t('search_ph') }}" autocomplete="off">
      <kbd>/</kbd>
  ...
  {% if not items %}<div class="empty"><b>∅</b>{{ t('no_results') if q or active_category else t('no_items') }}</div>{% endif %}
  <section class="list">
  {% for item in items %}
    {% set link = item.source_type == 'link' %}
    <a class="rc" style="--i:{{ loop.index0 }}" href="{{ item.source if link else media(item.source) }}"{% if link %} target="_blank" rel="noopener noreferrer"{% endif %}>
      <div class="rc-thumb">C31K{% if item.thumbnail %}<img src="{{ media(item.thumbnail) }}" alt="" loading="lazy">{% endif %}
        <span class="rank">#{{ loop.index }}</span>
        {% if now - item.created_at < new_window %}<span class="newtag">{{ t('new') }}</span>{% endif %}
      </div>
      <div class="rc-main">
        {% if item.thumbnail %}<img class="bgimg" src="{{ media(item.thumbnail) }}" alt="" loading="lazy">{% endif %}
        <div class="rc-txt">
          <h3>{{ item.title }}</h3>
          <div class="sub">{{ categories.get(item.category, item.category) }} · <b>{{ 'URL' if link else 'File' }}</b></div>
        </div>
        <div class="rc-side">
          <div class="act"><strong>{{ t('open') if link else t('download') }}</strong><small>{{ categories.get(item.category, item.category) }}</small></div>
          <span class="arrow">{{ '↗' if link else '↓' }}</span>
        </div>
      </div>
    </a>
  {% endfor %}
...
{% block content %}<main class="auth"><div class="panel sm">
  ...
  <h1>{{ t('login') }}</h1>
  <form method="post"><input name="username" placeholder="{{ t('username') }}" required autofocus autocomplete="username"><input name="password" type="password" placeholder="{{ t('password') }}" required autocomplete="current-password"><button class="btn full">{{ t('continue') }}</button></form>
...
{% block content %}<main class="auth"><div class="panel sm">
  ...
  <form method="post"><input name="username" placeholder="{{ t('username') }}" required autofocus maxlength="40" autocomplete="username"><input name="password" type="password" placeholder="{{ t('password') }}" required autocomplete="new-password"><button class="btn full">{{ t('register') }}</button></form>
